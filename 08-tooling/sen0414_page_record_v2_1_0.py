...
rec = {"widgets": W, "sections": [(i, n["level"], n["label"], "s-" + n["id"], n["section_iri"]) for i, n in enumerate(d["nodes"], 1)],
       "python": d["python"], "quiz": json.load(open(os.path.join(P, "quiz_v1_0_0.json")))}
json.dump(rec, open(os.path.join(P, "build_record_v%s.json" % PV), "w"), indent=1)
json.dump({"widgets": t["widgets"], "gates": t["gates"]}, open(os.path.join(P, "test_results_abox_v%s.json" % PV), "w"), indent=1)
