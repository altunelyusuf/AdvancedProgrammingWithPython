...
Each block names its file and hash. Usage: sen0414_page_build_v4_1_0.py <NN>"""
...
T = open(os.path.join(REPO, "08-tooling", os.environ.get("PAGE_TEMPLATE", "course_page_template_v9_0_0.html"))).read()
...
h = (T.replace("__TITLE__", d["title"]).replace("__COURSE__", d["course"]["line"]).replace("__SUB__", d["course"]["chapter_sub"].replace("{n}", str(d["chapter"])))
      .replace("__PY__", d["python"]).replace("__DATA__", safe(d)).replace("__QUIZ__", safe(json.load(open(os.path.join(P, "quiz_v1_0_0.json")))))
...
open(out, "w").write(h); json.dump(log, open(os.path.join(P, "corpus_manifest_v%s.json" % PV), "w"), indent=1)
