...
Usage: sen0414_page_data_v2_1_0.py <NN> <python>  ->  08-tooling/chNN-page/page_data_v2.json"""
...
COURSE = json.load(open(os.path.join(REPO, "08-tooling", "course_page_config_v1_2_0.json")))
DISC = os.path.join(REPO, "08-tooling", "ch%s-page" % N, "discussion_v1_0_0.json")
VIS = os.path.join(REPO, "08-tooling", "ch%s-page" % N, "visuals_v1_0_0.json")
data = {"visuals": json.load(open(VIS)) if os.path.exists(VIS) else {}, "course": COURSE, "discussion": json.load(open(DISC)) if os.path.exists(DISC) else [], "findings": findings, "unit": None,
