...
page_path = os.environ.get("PAGE", os.path.join(REPO, "03-materials", N, "page", "sen0414_%s_page_v%s.html" % (N, os.environ.get("PAGE_VER", "7_0_0"))))
...
with sync_playwright() as p:
    ...
    GROUP = {"taxonomy": "maps", "map": "maps", "ontograph": "maps", "play": "lab", "codelab": "lab", "sparql": "lab", "browse": "questions", "expert": "questions", "checks": "questions", "howto": "about", "arch": "about", "mission": "about", "prov": "about", "mydata": "about"}
    def view(k):
        g = GROUP.get(k)
        if g:
            pg.click('#groups [data-tab="%s"]' % g); pg.click('[data-pane]:not([hidden]) .viewsub [data-tab="%s"]' % k) if not pg.locator('[data-pane="%s"]' % k).is_visible() else None
        else:
    ...
    view("taxonomy"); feat("taxonomy diagram", pg.locator("#taxo svg g.n").count() == len(nodes))
    # widgets
    for n in nodes:
    ...
    pg.click('#groups [data-tab="agents"]'); pg.wait_for_timeout(200)
    ...
    kept = pg.locator("#%s-log .msg" % target).count(); pg.reload(); pg.click('#groups [data-tab="agents"]'); pg.wait_for_timeout(400)
