#!/usr/bin/env python3
"""Browser tests for version 5 - version 4's checks, plus: the page's own thread never blocks for long, and an endless program is stopped while the page stays responsive ( (version 3's rule checks plus the agents' toolkit) of a SEN0414 page - the owner's rules of 2026-09-25 made checkable:
...
page_path = os.environ.get("PAGE", os.path.join(REPO, "03-materials", N, "page", "sen0414_%s_page_v%s.html" % (N, os.environ.get("PAGE_VER", "5_0_0"))))
...
with sync_playwright() as p:
    ...
    def ask(q):
        ...
        pg.wait_for_function("([id,n])=>{const m=document.querySelectorAll('#'+id+'-log .msg');return m.length>=n+2&&!m[m.length-1].textContent.includes('searching')}", arg=[a["id"], before], timeout=240000)
    ...
    pg.click('#groups [data-tab="play"]'); pg.fill("#pcode", "while True:\n    pass"); t0 = time.time(); pg.click("#prun"); pg.wait_for_timeout(1500)
    t1 = time.time(); pg.click('#groups [data-tab="glossary"]'); responsive = (time.time() - t1) < 1.0 and pg.locator('[data-pane="glossary"]').is_visible()
    pg.click('#groups [data-tab="play"]'); pg.wait_for_function("()=>document.getElementById('pout').textContent.startsWith('Stopped after')", timeout=40000); stopped_in = time.time() - t0
