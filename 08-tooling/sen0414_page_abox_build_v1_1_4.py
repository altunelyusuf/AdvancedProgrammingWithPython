...
SERVES = OBJ.pop("_serves", []); OBJ.pop("_note", None); OBJ.pop("_version", None)  # metadata keys, not objectives - removed before anything reads OBJ
tr = json.load(open(os.path.join(REPO, "08-tooling", "ch%s-page" % N, "test_results_abox_v%s.json" % PV)))
...
<%s/page> a owl:Ontology ; rdfs:label "SEN0414 chapter %d - RDODI Stage 4 page ABox"@en ; owl:versionInfo "1.0.0" ;
    ...
    dcterms:identifier "sen0414_ch%s_page_abox_v1_0_0" ; prov:wasGeneratedBy <http://example.org/sen0414/activity/ch%s-rdodi-run> ;
...
pg:Page a ipo:Page, ipo:InteractiveLearningSurface ; rdfs:label "Chapter %d interactive page"@en ;
    ...
    sen0414:navigationRule "BP-D34: %d sections falls in 26-50, so a grouped permanent top bar with a sub-section sidebar; the named single flowing anchor-page variation of procedure v1.6.0 section 5.3." ;
    sen0414:pythonUsed "%s" .
...
for o, lv, t, sid, s in rec["sections"]:
    L.append('pg:%s a ipo:ChapterSection ; rdfs:label "%s"@en ; dcterms:source <%s> ; sen0414:anchor "#%s" .' % (sid.replace("-", "_"), q(t), s, sid))
