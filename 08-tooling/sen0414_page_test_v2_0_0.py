#!/usr/bin/env python3
"""Browser tests for version 2 of a SEN0414 chapter page: each capability the owner asked for is exercised
...
page_path = os.environ.get("PAGE", os.path.join(REPO, "03-materials", "ch%s" % N, "page", "sen0414_ch%s_page_v2_0_0.html" % N))
...
with sync_playwright() as p:
    ...
    pg.click('#tabs [data-tab="play"]'); pg.select_option("#pex", "first"); pg.click("#pload"); pg.fill("#pin", "Ayse|20"); pg.click("#prun")
    pg.wait_for_function("()=>document.getElementById('pout').textContent!=='running...'", timeout=60000); out = pg.text_content("#pout")
    feat("playground runs a full program with input()", "It is good to meet you, Ayse." in out and "You will be 21 in a year." in out, out.replace("\n", " | ")[:120])
