#!/usr/bin/env python3
"""Generic RDODI Stages 1-3 builder for a SEN0414 chapter, driven by a chapter data module.
Usage: sen0414_rdodi_build_v1_0_1.py <data-module> <verified-at>
...
@prefix sen0414: <http://example.org/sen0414#> .
...
def header(part, label):
    ident = "sen0414_ch%s_%s_v1_0_0" % (N, part)
    ...
    prov:wasGeneratedBy <http://example.org/sen0414/activity/ch%s-rdodi-run> ;
...
def research():
    L = [PFX, header("research", "SEN0414 chapter %d - RDODI Stage 1 research artefact" % D.CH),
         'chx:Research a res:ResearchProject ; rdfs:label "%s"@en ;' % q(D.TITLE),
         '    sen0414:methodology "Primary source first: the chapter\'s named concepts from the textbook ontology built over the published chapter. Then the official documentation for the current release and the PEPs the chapter\'s topics touch. Every source opened and read on %s; every claim taken from text actually read; every behaviour executed under Python 3.14.4." ;' % VER,
...
def tbox():
    L = [PFX, header("tbox", "SEN0414 chapter %d domain ontology - TBox" % D.CH)]; seen = set()
...
def abox():
    L = [PFX, header("abox", "SEN0414 chapter %d domain ontology - ABox" % D.CH)]
    ...
    L.append('''chx:Artifact a owl:NamedIndividual, rd:DomainOntologyArtifact ; rdfs:label "SEN0414 chapter %d domain ontology"@en ;
...
chx:CQ1 a owl:NamedIndividual, rd:CompetencyQuestion ; rdfs:label "Which constructs decide which block runs, and on what kind of value?"@en .
chx:CQ2 a owl:NamedIndividual, rd:CompetencyQuestion ; rdfs:label "Which concepts reach beyond the 3rd edition into current Python, and on what source?"@en .
chx:Provenance a owl:NamedIndividual, rd:ResearchSubjectInput ; rdfs:label "Derived from the chapter %d research artefact"@en ;
    skos:definition "Concepts are corpus-derived from the 3rd edition's chapter %d through the Stage 1 research artefact; the modern-practice branch comes from that artefact's secondary sources."@en ;
...
chx:Env a owl:NamedIndividual, rd:ResolutionEnvironment ; rdfs:label "CPython 3.14.4"@en ;
    ...
    dcterms:source "The execution run recorded with this build, 08-tooling/sen0414_rdodi_build_v1_0_1.py" .
''' % (D.CH, BASE, D.CH, D.CH, D.CH, BASE))
    return "\n".join(L) + "\n"

def shacl():
    return PFX + header("shacl", "SEN0414 chapter %d domain ontology - shapes" % D.CH) + '''chx:ExemplarShape a sh:NodeShape ; sh:targetClass owl:NamedIndividual ;
...
def document():
    ...
    L = [PFX, header("document", "SEN0414 chapter %d - RDODI Stage 3 document" % D.CH),
         'chx:Document a doc:ReportSection ; rdfs:label "%s"@en ; skos:definition "This document renews chapter %d of the 3rd edition for an advanced course (Sweigart, 2025)." ; dcterms:source <%s/research> ;' % (q(D.TITLE), D.CH, BASE),
...
for name, text in (("research", research()), ("domain_tbox", tbox()), ("domain_abox", abox()), ("domain_shacl", shacl()), ("document", document()[0])):
    open(os.path.join(OUT, "sen0414_ch%s_%s_v1_0_0.ttl" % (N, name)), "w").write(text)
